...
$LanguageLabels = LanguageValues::
    join("language_labels", "language_values.label_id", "=", "language_labels.id")
    ->where("language_values.lang_id", "=", session('_LANG_ID'))
    ->where('language_labels.key_id', 'like', '%[_validate_%')
    ...
    ->get()->toArray();
$newdata      = [];
$stringRemove = '[_validate_';
foreach ($LanguageLabels as $key => $value) {
    $key_id    = $value["key_id"];
    $key_value = $value["value"];
    if (strpos($key_id, '[_validate_array_') !== false) {
        $stringRemove = '[_validate_array_';
...
return array_merge($default, $newdata);
