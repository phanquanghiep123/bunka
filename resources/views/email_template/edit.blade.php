    ...
    <div class="col-lg-12 grid-margin">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page"><?php echo $record['key_id']; ?></li>
            </ol>
        </nav>
        <h4><?php echo $record['key_id']; ?></h4>
    </div>
</div>
<form class="form form-ajax" method="post" action="{{ $edit_url }}" enctype="multipart/form-data" autocomplete="off">
                        ...
                        <div class="form-row">
                            <div class="col-12">
                                <div class="form-group">
                                    <label>_KEY_</label>
                                    <input type="text" validate="true" data-validate="required" value="<?php echo $record['key_id']; ?>"  name="key_id" class="form-control" placeholder="_KEY_">
                                </div>
                            </div>
                        </div>
                            ...
                            <?php foreach($languages as $key => $language): ?>
                                ...
                                <div class="form-row">
                                    <div class="col-12">
                                        <div class="form-group">
                                            <label>_TITLE_</label>
                                            <input type="text" name="title[{{ $language->id }}]" value="<?php echo @$record['title_'.$language->id]; ?>" class="form-control">
                                        </div>
                                    </div>
                                    <div class="col-12">
                                        <div class="form-group">
                                            <label>_CONTENT_</label>
                                            <textarea class="form-control content" rows="10" name="content[{{ $language->id }}]"><?php echo @$record['content_'.$language->id]; ?></textarea>
                                        </div>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            <select name="status" class="form-control" validate="true" data-validate="required">
                                <option value="1" <?php echo @$record['status'] == '1' ? 'active' : ''; ?>>_SAVE_ _ACTIVE_</option>
                                <option value="0" <?php echo @$record['status'] == '0' ? 'active' : ''; ?>>_SAVE_ _DEACTIVE_</option>
                            </select>
                        </div>
