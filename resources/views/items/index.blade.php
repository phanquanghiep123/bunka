                    ...
                    <div class="input-group">
                        <div class="input-group-prepend">
                            <select class="form-control" name="ColumnName">
                                ...
                                <option value="2">Item Class</option>
                                <option value="">Price Pattern</option>
                                <option value="3">Price Key</option>
                            </select>
                        </div>
...
$(function () {
    ...
    .on('submit', '#modal-modify form', function () {
        ...
        .done(function () {
            var url = formData.Id ? "{{route('admin.items.update')}}" : "{{route('admin.items.store')}}";
    ...
    .on('click', '[data-toggle="modal"]', function (e) {
        ...
        e.preventDefault();
        if (data.id) {
            $.get("{{route('admin.items.show', ['id' => ''])}}/" + data.id, function (res) {
    ...
    .on('change submit', '#filterForm', function () {
        ...
        // $('[name="ColumnName"] > option').each(function () {
        //     LaravelDataTables.dataTableBuilder.column($(this).attr('value')).search('');
        // });
        ...
        window.filterTimeout = setTimeout(function () {
            if (filterData.ColumnValue && filterData.ColumnName) {
                LaravelDataTables.dataTableBuilder.column(filterData.ColumnName).search(filterData.ColumnValue, true, false);
            }
