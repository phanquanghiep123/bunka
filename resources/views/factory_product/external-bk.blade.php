    ...
    <div class="col-lg-12 grid-margin">
        ...
        <table id="example" class="table table-bordered dt-responsive nowrap" style="width:100%">
            <thead>
                <tr>
                    <th>#</th>
                    <th>[_date_of_recording_]</th>
                    <th>[_document_number_]</th>
                    <th>[_document_date_]</th>
                    <th>[_code_works_] </th>
                    <th>[_reciprocal_account_]</th>
                    <th>[_number_of_debt_incurred_]</th>
                    <th>[_the_number_arises_there_]</th>
                    <th>[_outstanding_balance_]</th>
                    <th>[_credit_]</th>
                    <th>[_explains_]</th>
                </tr>
            </thead>
            <tbody>
                @if(isset($models))
                    @foreach ($models as $key => $value)
                        <tr>
                            <td>{{ $key+1 }}</td>
                            <td><?php echo date('d/m/Y',strtotime(@$value->date_of_recording)); ?></td>
                            <td>{{ $value->document_number }}</td>
                            <td><?php echo @$value->document_date != null ? date('d/m/Y',strtotime(@$value->document_date)) : ''; ?></td>
                            <td>{{ $value->building_code }}</td>
                            <td>{{ $value->reciprocal_account }}</td>
                            <td>{{ number_format($value->number_of_debt_incurred) }} VND</td>
                            <td>{{ number_format($value->the_number_arises_there) }} VND</td>
                            <td>{{ number_format($value->outstanding_balance) }} VND</td>
                            <td>{{ number_format($value->credit) }} VND</td>
