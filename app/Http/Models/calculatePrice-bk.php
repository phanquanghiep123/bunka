...
class calculatePrice 
{
	...
	function __construct($ClassKey = null, $ItemId = null)
	{
		
		$this->Class         = $this->getMatrix("ClassProduct"); 
		$this->ClassPattern  = $this->getMatrix("ClassPattern"); 
		
		$this->_rateDefault  = \App\Http\Models\Mstclass::where("Class", "=", 2)->orderby(DB::raw("CAST(Value2 as Decimal)"), "DESC")->first();		
	}
    ...
    public function GetPriceByMatrix ($PricePatternKey,$Matrix,$ClassFactoryPriceKey){
        ...
        if($data) return $data[0]->MatrixPrice;
        else return "";
    }
